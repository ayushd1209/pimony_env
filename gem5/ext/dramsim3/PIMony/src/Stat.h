...
namespace pimony
{
  typedef struct MemoryIOStat
  {
    ...
    static std::vector<StatType> get_stat_types()
    {
      return {
          ...
          StatType::MemoryReadBandwidth,
          StatType::MemoryWriteBandwidth,
          StatType::PIMReads,
          StatType::PIMReadBandwidth,
      };
    }
  ...
  typedef struct OperationStat
  {
    ...
    void update_stat(TileStat tile_stat)
    {
      ...
      if (tile_stat.start_cycle < start_cycle)
        start_cycle = tile_stat.start_cycle;
