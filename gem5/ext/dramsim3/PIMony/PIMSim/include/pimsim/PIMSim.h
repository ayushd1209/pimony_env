...
class PIMSim {
   public:
    PIMSim(const std::string &config_file, const std::string &output_dir,
    ...
    bool AddTransaction(uint64_t hex_addr, int req_type, uint32_t num_macs,
                        void *original_req);
   ...
   private:
    ...
    void PushToPendingQueue(uint64_t addr, TransactionType req_type,
                            void *original_req);
};
class PIMSim::ResponseQueue {
   public:
