...
class ChannelState {
   ...
   private:
    ...
    void UpdateOtherSubarraySameBankTimingForPIM(
        const Address &addr,
        const std::vector<std::pair<CommandType, int>> &cmd_timing_list,
        uint64_t clk);

    void UpdateOtherSubarraySameBankTiming(
        const Address &addr,
        const std::vector<std::pair<CommandType, int>> &cmd_timing_list,
        uint64_t clk);

    void UpdateSameBankgroupTiming(
        const Address &addr,
        const std::vector<std::pair<CommandType, int>> &cmd_timing_list,
        uint64_t clk);
