...
namespace dramsim3 {
class PIMController : public Controller {
   public:
    PIMController(int channel, const Config &config, const Timing &timing);
    ...
    std::pair<uint64_t, TransactionType> ReturnDoneTrans(
   ...
   private:
    ...
    PIMCommandQueue pim_cmd_queue_;
    Refresh refresh_;
    ...
    // mac operation state for each bankgroup
