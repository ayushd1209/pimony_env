...
class Config {
   public:
    ...
    uint64_t MakeAddress(int channel, int rank, int bankgroup, int bank,
                         int row, int col);
    uint64_t ReverseAddressMapping(const Address &addr) const;
    ...
    int mem_first_threshold;
    std::string compute_mode;
    std::string bank_mode;
    ...
    bool IsLPDDR() const {
        return (protocol == DRAMProtocol::LPDDR3 ||
                protocol == DRAMProtocol::LPDDR4 ||
