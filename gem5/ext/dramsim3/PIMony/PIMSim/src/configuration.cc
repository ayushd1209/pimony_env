...
uint64_t Config::MakeAddress(int channel, int rank, int bankgroup, int bank,
                             int row, int col) {
    uint64_t addr = 0;
...
Address Config::AddressMapping(uint64_t hex_addr) const {
    ...
    // PrintError("shift_bits:", shift_bits, "ch_pos", ch_pos, "ra_pos", ra_pos,
    // "bg_pos", bg_pos,
    //            "ba_pos", ba_pos, "ro_pos", ro_pos, "co_pos", co_pos);

    // PrintImportant("AddressMapping hexaddr:", HexString(origin_addr), "ch:",
    // channel, "ra:", rank,
...
uint64_t Config::ReverseAddressMapping(const Address &addr) const {
    uint64_t hex_addr = 0;
...
void Config::CalculateSize() {
    ...
    devices_per_rank = IsGDDR() || IsLPDDR() ? 1 : bus_width / device_width;
    int page_size = (IsGDDR()) ? columns / BL * bus_width / 8
                               : columns * device_width / 8;  // in bytes
    ...
    if (megs_per_rank > channel_size) {
        std::cout << "WARNING: Cannot create memory system of size "
                  ...
                  << megs_per_rank << "MB instead!" << std::endl;
        ranks = 1;
...
DRAMProtocol Config::GetDRAMProtocol(std::string protocol_str) {
    std::map<std::string, DRAMProtocol> protocol_pairs = {
        ...
        {"LPDDR5", DRAMProtocol::LPDDR5},
        {"LPDDR5X", DRAMProtocol::LPDDR5X},
        {"HBM", DRAMProtocol::HBM},
        {"HBM2", DRAMProtocol::HBM2}};  // gsheo: remove HMC
...
void Config::InitDRAMParams() {
    ...
    rows_in_subarray = GetInteger("dram_structure", "rows_in_subarray", 1 << 9);
    ...
    if (protocol == DRAMProtocol::GDDR5) {
        ...
        BL = (BL == 0) ? 8 : BL;
               ...
               protocol == DRAMProtocol::LPDDR5 ||
               protocol == DRAMProtocol::LPDDR5X) {
...
void Config::InitPowerParams() {
    ...
    // * time(in cycles) units are V * mA * Cycles and if we convert cycles
    // to ns then it's exactly pJ in energy and because a command take
    // effects on all devices per rank, also multiply that number
    ...
    int page_size = (IsGDDR())
                        ? columns / BL * bus_width
                        : ((IsLPDDR()) ? columns * device_width
                                       : columns * bus_width);  // in bits
    int read_size = BL * device_width;                          // in bits

    double readres_scaling =
        16 * (banks * devices_per_rank * ranks) / read_size;
    if (compute_mode == "ASYNC") {
        readres_scaling = 16 * (banks_per_group) / read_size;
    }
    int gwrite_scaling = page_size / read_size;
    PrintImportant("readres scaling", readres_scaling, "gwrite scaling",
                   gwrite_scaling);

    // gsheo: for PIM command power
    gwrite_energy_inc = gwrite_scaling * read_energy_inc;
    gact_energy_inc = 4 * act_energy_inc;
    if (compute_mode == "ALL_BANK") {
        comp_energy_inc = 4 * read_energy_inc * ranks;
    } else if (compute_mode == "ASYNC") {
        comp_energy_inc = read_energy_inc;
    }
...
void Config::InitSystemParams() {
    ...
    ranks = GetInteger("system", "ranks", 1);
    ...
    scheduling_policy = reader.Get("system", "scheduling_policy", "PIM_FIRST");
    mem_first_threshold = GetInteger("system", "mem_first_threshold", 8);
    compute_mode = reader.Get("system", "compute_mode", "ALL_BANK");
    bank_mode = reader.Get("system", "bank_mode", "SINGLE");
...
void Config::InitTimingParams() {
    ...
    gwrite_delay = (tCCD_S)*gwrite_scaling;
