...
class BaseDRAMSystem {
   public:
    ...
    virtual bool WillAcceptTransaction(uint64_t hex_addr,
                                       TransactionType req_type) const = 0;
    virtual bool AddTransaction(uint64_t hex_addr,
                                TransactionType req_type) = 0;
    virtual bool AddTransaction(uint64_t hex_addr, TransactionType req_type,
                                uint32_t num_macs) = 0;
    virtual void ClockTick() = 0;
    int GetChannel(uint64_t hex_addr) const;

    std::function<void(uint64_t req_id)> read_callback_, pim_callback_,
        write_callback_;
...
class JedecDRAMSystem : public BaseDRAMSystem {
   public:
    ...
    bool WillAcceptTransaction(uint64_t hex_addr,
                               TransactionType req_type) const override;
    bool AddTransaction(uint64_t hex_addr, TransactionType req_type) override;
    bool AddTransaction(uint64_t hex_addr, TransactionType req_type,
                        uint32_t num_macs) override;
