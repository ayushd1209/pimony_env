...
class PIMCommandQueue {
   public:
    PIMCommandQueue(int channel_id, const Config &config,
    ...
    void ClockTick();
    bool WillAcceptCommand(CommandType cmd_type, int rank, int bankgroup,
                           int bank) const;
    bool AddCommand(Command cmd);
    void ErasePIMCommand(const Command &cmd);

    bool QueueEmpty() const;
    int QueueUsage() const;
    bool PIMQueueEmpty() const;
    int GetPIMQueueSize() const;
    void FinishGwrite() {
        is_gwriting_ = false;
        remain_slack_ = 0;
    }
    void PrintAllQueue() const;  // debugging
   ...
   private:
    ...
    bool CanMeetRefreshDeadline(const CMDIterator cmd_it,
                                std::pair<int, int> refresh_slack);
    // for debug
