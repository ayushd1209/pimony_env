...
SimpleStats::SimpleStats(const Config &config, int channel_id)
    ...
    // counter stats
    InitStat("pim_cycles", "counter",
             "Number of PIM cycles that pim cmd queue is not empty");
    ...
    InitStat("num_readres_done", "counter",
             "Number of read_res requests issued");
    ...
    // pim stats
    InitStat("num_early_refresh_cuz_pim", "counter",
             "Number of Early REFRESH commands due to PIM");
    InitStat("num_yield_for_rdwr", "counter", "Number of yield for Read/Wrtie");
    InitStat("num_gwrite_cmds", "counter", "Number of GWRITE commands");
    InitStat("num_gact_cmds", "counter", "Number of GACT commands");
    InitStat("num_comp_cmds", "counter", "Number of COMP commands");
    InitStat("num_readres_cmds", "counter", "Number of READRES commands");

    InitStat("num_interleaved_write_cmds", "counter",
             "Number of Interleaved WRITE commands to the GWRITE/COMP-READRES");
    InitHistoStat("gwrite_latency", "PIM GWRITE request latency (cycles)", 0,
                  400, 50);

    // used for dual buffer
    // interleaved command count in pim operation slack time
    InitStat(
        "num_parallel_prec_cmds", "counter",
        "Number of PRECHARGE commands that interleaved during PIM operation");
    InitStat(
        "num_parallel_act_cmds", "counter",
        "Number of ACTIVATE commands that interleaved during PIM operation");
    InitStat("num_parallel_read_cmds", "counter",
             "Number of READ commands that interleaved during PIM operation");
    InitStat("num_parallel_write_cmds", "counter",
             "Number of WRITE commands that interleaved during PIM operation");
    ...
    // for PIM command
    InitStat("gwrite_energy", "double", "Gwrite energy");
    InitStat("gact_energy", "double", "Gact energy");
    InitStat("comp_energy", "double", "Compute energy");
    InitStat("readres_energy", "double", "Readres energy");
    ...
    InitVecStat("sref_energy", "vec_double", "SREF energy", "rank",
                config_.ranks);
    // for PIM
    InitVecStat("gact_stb_energy", "vec_double", "PIM Active standby energy",
                "rank", config_.ranks);
    InitVecStat("pim_pre_stb_energy", "vec_double",
                "PIM Precharge standby energy", "rank", config_.ranks);

    // Histogram stats
    InitHistoStat("read_latency", "Read request latency (cycles)", 0, 400, 10);
    ...
    InitStat("average_gwrite_latency", "calculated",
             "Average gwrite request latency (cycles)");
...
void SimpleStats::UpdateEpochStats() {
    ...
    // per-command energy for pim
    doubles_["gwrite_energy"] =
        epoch_counters_["num_gwrite_cmds"] * config_.gwrite_energy_inc;
    doubles_["gact_energy"] =
        epoch_counters_["num_gact_cmds"] * config_.gact_energy_inc;
    doubles_["comp_energy"] =
        epoch_counters_["num_comp_cmds"] * config_.comp_energy_inc;
    doubles_["readres_energy"] =
        epoch_counters_["num_readres_cmds"] * config_.readres_energy_inc;
    ...
    for (int i = 0; i < config_.ranks; i++) {
        ...
        double pim_act_stb = 0;
        double pim_pre_stb = 0;
        ...
        vec_doubles_["gact_stb_energy"][i] = pim_act_stb;
        vec_doubles_["pim_pre_stb_energy"][i] = pim_pre_stb;
        background_energy +=
            act_stb + pre_stb + sref_energy + pim_act_stb + pim_pre_stb;
    }

    UpdateHistoBins();

    // calculated stats
    uint64_t total_reqs = epoch_counters_["num_reads_done"] +
                          epoch_counters_["num_writes_done"] +
                          epoch_counters_["num_readres_done"];
    ...
    double total_energy = doubles_["act_energy"] + doubles_["read_energy"] +
                          ...
                          doubles_["refb_energy"] + doubles_["gwrite_energy"] +
                          doubles_["gact_energy"] + doubles_["comp_energy"] +
                          doubles_["readres_energy"] + background_energy;
    ...
    calculated_["average_gwrite_latency"] =
        GetHistoAvg(epoch_histo_counts_.at("gwrite_latency"));
...
void SimpleStats::UpdateFinalStats() {
    ...
    // gsheo: for pim commands
    doubles_["gwrite_energy"] =
        counters_["num_gwrite_cmds"] * config_.gwrite_energy_inc;
    doubles_["gact_energy"] =
        counters_["num_gact_cmds"] * config_.gact_energy_inc;
    doubles_["comp_energy"] =
        counters_["num_comp_cmds"] * config_.comp_energy_inc;
    doubles_["readres_energy"] =
        counters_["num_readres_cmds"] * config_.readres_energy_inc;
    ...
    // calculated stats
    uint64_t total_reqs = counters_["num_reads_done"] +
                          counters_["num_writes_done"] +
                          counters_["num_readres_done"];
    ...
    calculated_["average_gwrite_latency"] =
        GetHistoAvg(histo_counts_.at("gwrite_latency"));
    calculated_["average_interarrival"] =
        GetHistoAvg(histo_counts_.at("interarrival_latency"));

    // gsheo: total energy breakdown
    PrintDebug("Total energy breakdown");
    PrintDebug("act_energy", doubles_["act_energy"]);
    PrintDebug("read_energy", doubles_["read_energy"]);
    PrintDebug("write_energy", doubles_["write_energy"]);
    PrintDebug("ref_energy", doubles_["ref_energy"]);
    PrintDebug("refb_energy", doubles_["refb_energy"]);
    PrintDebug("gwrite_energy", doubles_["gwrite_energy"]);
    PrintDebug("gact_energy", doubles_["gact_energy"]);
    PrintDebug("comp_energy", doubles_["comp_energy"]);
    PrintDebug("readres_energy", doubles_["readres_energy"]);
    PrintDebug("background_energy", background_energy);
