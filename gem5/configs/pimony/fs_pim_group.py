# Step 6 config — GROUP SCOPING: 3 MACs, one completion.
# Identical wiring to fs_fence_e2e.py; only the workload differs, so the two are
# directly comparable (1 MAC -> 1 interrupt vs 3 MACs -> 1 interrupt).
#
# Run (inside gem5-container):
#   build/RISCV/gem5.opt -d m5out/group_timing --debug-flags=DRAMsim3 \
#       configs/pimony/fs_pim_group.py timing 2>&1 | tee m5out/group_timing/run.log
...
system.workload.bootloader = "tests/test-progs/pim_group/pim_group"
...
print("Beginning Step 6 (PIM group scoping: 3 MACs, 1 completion)!")
